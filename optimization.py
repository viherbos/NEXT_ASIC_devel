...
class compressors(object):
    def __init__(self, mosfet_params, time_unit, caps, R, bias, max_pe, lsb_pe, pulse_length,
                 signal, PE_array, PE_inc, MAX_PE):
        ...
        self.I1 = integrator(KPP_2n = mosfet_params['KPP_2n'],
                                ...
                                time_unit = self.tu,
                                cap = self.cap1)
        self.I3 = LOG_integrator(KPP_2n = mosfet_params['KPP_2n'],
                                W_L = mosfet_params['W_L'],
                                VTH = mosfet_params['VTH'],
                                time_unit = self.tu,
                                cap  = self.cap1
    ...
    def get_integral(self):
        for i in range(1,len(self.PE_array)):
            Iout,vGS_C = self.I1.ideal(self.I_o, self.I_a, self.signal[i*self.p_length:(i+1)*self.p_length])
